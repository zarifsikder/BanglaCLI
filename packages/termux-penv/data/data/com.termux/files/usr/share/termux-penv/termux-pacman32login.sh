...
SCRIPTNAME=termux-pacman32login.sh
PROOT=$PREFIX/bin/proot
HOME=$PREFIX/../home
NEWROOT=$PREFIX/var/lib/termux_penv/pacman-chroot32/usr

if [ ! -d "$NEWROOT" ]; then
    echo "ERROR: No chroot installed. Install it with termux-penv install termux-pacman32"
    exit 1
fi

show_usage() {
    echo "Usage: $SCRIPTNAME [command]"
    echo "termux-penv login termux-pacman32: Setup a chroot to use Termux bootstrap in isolated chroot"
    echo ""

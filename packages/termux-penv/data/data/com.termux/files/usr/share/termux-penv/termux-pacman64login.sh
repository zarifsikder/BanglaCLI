...
SCRIPTNAME=termux-pacman64login.sh
PROOT=$PREFIX/bin/proot
HOME=$PREFIX/../home
NEWROOT=$PREFIX/var/lib/termux_penv/pacman-chroot64/usr

if [ ! -d "$NEWROOT" ]; then
    echo "ERROR: No chroot installed. Install it with termux-penv install termux-pacman64"
    exit 1
fi

show_usage() {
    echo "Usage: $SCRIPTNAME [command]"
    echo "termux-penv login termux-pacman64: Setup a chroot to use Termux bootstrap in isolated chroot"
    echo ""

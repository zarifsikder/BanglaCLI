...
SCRIPTNAME=termux64login.sh
PROOT=$PREFIX/bin/proot
HOME=$PREFIX/../home
NEWROOT=$PREFIX/var/lib/termux_penv/chroot64/usr

if [ ! -d "$NEWROOT" ]; then
    echo "ERROR: No chroot installed. Install it with termux-penv install termux64"
    exit 1
fi

show_usage() {
    echo "Usage: $SCRIPTNAME [command]"
    echo "termux-penv login termux64: Setup a chroot to use Termux bootstrap in isolated chroot"
    echo ""
